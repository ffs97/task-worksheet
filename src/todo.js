...
function addTracker(id) {
  $("#tracker-list").append(
    '<li class="tracker-id" id="' +
      trackerIdPrefix.substring(1) +
      id +
      '"><span>' +
      id +
      "</span></li>"
    ...
    update: function (_event, _ui) {
      ...
      taskBox.children(".task").each(function () {
        tasks.push(
          $(this)
            .attr("id")
            .substring(taskIdPrefix.length - 1)
        );
      });
...
function addProject(id) {
  ...
  var projectHTML = newProjectHTML
    ...
    .replaceAll(
      "{project_title}",
      getActiveTrackerInfo().project_info[id].title
    );
...
function addTask(projectId, taskId) {
  const taskInfo =
    todo.tracker_info[selectedTracker].project_info[projectId].task_info[
      taskId
    ];
...
function addTasks(projectId) {
  const taskIds =
    todo.tracker_info[selectedTracker].project_info[projectId].tasks;
  taskIds.forEach((id) => {
...
function setTaskStatus(status) {
  $("#item-status-selected-option .item-field-image").attr(
    "src",
    "img/status/" + status + ".png"
  );
  $("#item-status-selected-option .item-status-option-text").text(
    statusToTextMap[status]
  );
  $(".task.selected .task-status img").attr(
    "src",
    "img/status/" + status + ".png"
  );
}
...
function removeTaskAssignee(assigneeId) {
  const assignee = $(
    assigneeIdPrefix + assigneeId + " .item-assignee-value"
  ).text();
...
function addTaskAssignee(assignee) {
  ...
  $(assigneeIdPrefix + assigneeId + " .item-assignee-remove").click(
    function () {
...
function toggleArchiveActionItem(actionItemId) {
  var container = $(actionItemIdPrefix + actionItemId);
  if (container.hasClass("completed")) return;
  var archiveButton = container.find(
    ".action-item-buttons .action-item-archive"
  );
  ...
  if (container.hasClass("archived")) {
    ...
    $(actionItemIdPrefix + actionItemId).appendTo("#action-items-archived");
    $("#action-items-archived-toggle").show();
  } else {
    container.find("textarea").prop("disabled", false);
    archiveButton.prop("src", "img/archive.png");
    actionItemInfo.status = "new";
    $(actionItemIdPrefix + actionItemId).appendTo("#action-items-pending");
    if ($("#action-items-archived").children().length() == 0) {
      $("#action-items-archived-toggle").hide();
    }
  }
}

function toggleCompleteActionItem(actionItemId) {
  var container = $(actionItemIdPrefix + actionItemId);
  ...
  if (container.hasClass("completed")) {
    ...
    $(actionItemIdPrefix + actionItemId).appendTo("#action-items-completed");
    $("#action-items-completed-toggle").show();
  } else {
    actionItemInfo.status = "new";
    $(actionItemIdPrefix + actionItemId).appendTo("#action-items-pending");
    if ($("#action-items-completed").children().length() == 0) {
      $("#action-items-completed-toggle").hide();
    }
  }
}
...
function addActionItem(actionItemId) {
  ...
  var actionItemHTML = newActionItemHTML
    .replaceAll(
      "{action_item_id}",
      actionItemIdPrefix.substring(1) + actionItemId
    )
    .replaceAll("{action_item_title}", actionItemInfo.title);
  $("#action-items-pending").append(actionItemHTML);

  var container = $(actionItemIdPrefix + actionItemId);
  var actionItemTextarea = container.find(".action-item-textarea");
  actionItemTextarea.on("click blur keyup paste input", function () {
    actionItemInfo.title = actionItemTextarea.val();
    actionItemTextarea.css("height", "0");
    actionItemTextarea.css("height", $(this).prop("scrollHeight") + "px");
  });
  requestAnimationFrame(function () {
    actionItemTextarea.css("height", "0");
    actionItemTextarea.css(
      "height",
      actionItemTextarea.prop("scrollHeight") + "px"
    );
  });
  ...
  container
    .find(".action-item-buttons .action-item-archive")
    .click(function () {
      toggleArchiveActionItem(actionItemId);
    });
  container.find(".action-item-checkbox").click(function () {
    toggleCompleteActionItem(actionItemId);
  });
...
function createNewActionItem(title = "", status = "new") {
  ...
  taskInfo.action_item_info[actionItemId] = {
    id: actionItemId,
    title: "",
    status: "new",
  };
...
function uploadProjectToInfoBox() {
  ...
  $("#action-items-pending").empty();
  $("#action-items-completed").empty();
  $("#action-items-archived").empty();
  $("#item-status").css("display", "none");
  $("#info-assignees").css("display", "none");
  $("#info-actions").css("display", "none");
}

function toggleActionItemsWrapper(button, wrapper, force_hide = false) {
  const isVisible = force_hide || !wrapper.hasClass("collapsed");
  if (isVisible) {
    const h = wrapper.height();
    wrapper.css("height", h).animate({ height: 0 }, 200);
    wrapper.addClass("collapsed");
    button.css("font-weight", "400");
  } else {
    if (wrapper.find(".action-items-list").children().length > 0) {
      const h = wrapper.prop("scrollHeight");
      wrapper.css("height", 0).animate({ height: h }, 200, function () {
...
function uploadTaskToInfoBox() {
  ...
  $("#action-items-completed-toggle").hide();
  toggleActionItemsWrapper(
    $("#action-items-completed-toggle"),
    $("#action-items-completed-wrapper"),
    /*force_hide=*/ true
  );
  $("#action-items-archived-toggle").hide();
  toggleActionItemsWrapper(
    $("#action-items-archived-toggle"),
    $("#action-items-archived-wrapper"),
    /*force_hide=*/ true
  );
...
function addInfoBoxFunctions() {
  ...
  $("#action-items-completed-toggle").click(function () {
    toggleActionItemsWrapper($(this), $("#action-items-completed-wrapper"));
    toggleActionItemsWrapper(
      $("#action-items-archived-toggle"),
      $("#action-items-archived-wrapper"),
      /*force_hide=*/ true
  ...
  $("#action-items-archived-toggle").click(function () {
    toggleActionItemsWrapper($(this), $("#action-items-archived-wrapper"));
    toggleActionItemsWrapper(
      $("#action-items-completed-toggle"),
      $("#action-items-completed-wrapper"),
      /*force_hide=*/ true
  ...
  var pendingActionItemsListBox = $("#action-items-pending");
  pendingActionItemsListBox.sortable({
      ...
      $(".action-items-list")
        ...
        .each(function () {
          actionItems.push(
            $(this)
              .attr("id")
              .substring(actionItemIdPrefix.length - 1)
          );
        });
...
function getTodoString() {
  return JSON.stringify(todo, (_key, value) =>
    value instanceof Set ? [...value] : value
  );
}

function fromTodoString(todoString) {
  todo = JSON.parse(todoString, (key, value) =>
    key == "assignees" ? new Set(value) : value
  );
}
...
async function loadTodoFromHandler(ignoreErrors = false) {
  try {
    ...
    if (!ignoreErrors) {
      alert(
        "Error reading the TODO file. Please correct any issues in the file."
      );
    }
    return;
  }
  ...
  await set("file", todoFileHandle).catch((err) =>
...
async function tryLoadingLastFile() {
  ...
  if (todoFileHandle == undefined) {
    console.log(
      "Error reading file from key-value store. Please re-import it."
    );
    return;
  }
